...
namespace Megarender.StorageService.Middleware
{
    public class RequestResponseLoggingMiddleware {
        ...
        public async Task Invoke (HttpContext context) {            
            ...
            ) 
            {
                ...
                using (var responseBody = new MemoryStream ()) {
                    ...
                    {
                        case StorageClientErrorResponse r: 
                            _logger.LogError("{@request}", request); 
                            break;
                        case StorageServerErrorResponse r: 
        ...
        private async Task<StorageRequest> FormatRequest (HttpRequest request, Guid id) {
        ...
        private async Task<StorageResponse> FormatSuccessResponse (HttpResponse response) {                        
            var result = new StorageSuccessResponse();
        ...
        private async Task<StorageResponse> FormatClientErrorResponse (HttpResponse response) {
            var result = new StorageClientErrorResponse();
            response.Body.Seek (0, SeekOrigin.Begin);
            var text = await new StreamReader (response.Body).ReadToEndAsync ();        
            if(!String.IsNullOrEmpty(text)) result.Body = JsonConvert.DeserializeObject<Dictionary<string,object>>(text);
            response.Body.Seek (0, SeekOrigin.Begin);
        ...
        private async Task<StorageResponse> FormatServerErrorResponse (HttpResponse response) {
            var result = new StorageServerErrorResponse();
            response.Body.Seek (0, SeekOrigin.Begin);
            var text = await new StreamReader (response.Body).ReadToEndAsync ();
            if(!String.IsNullOrEmpty(text)) result.Body = JsonConvert.DeserializeObject<Dictionary<string,object>>(text);
            response.Body.Seek (0, SeekOrigin.Begin);
